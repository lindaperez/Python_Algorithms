...
M_10 = Map()
M_10.intersections = {0: [0.7798606835438107, 0.6922727646627362],
...
def shortest_path(M,start,goal):
    ...
    def reconstruct_path(start, camefrom, current):
        total_path = [current]
        while current != start:
    ...
    def a_star(M,start, goal):
        ...
        gScore = {node: float("inf") for node in M.intersections.keys()}
        gScore[start] = 0.0
        #    // For node n, fScore[n] := gScore[n] + h(n).
        fScore = {node: float("inf") for node in M.intersections.keys()}
        dist_s_g =distance(M.intersections[start], M.intersections[goal])
        fScore[start] = dist_s_g

        heap = [(float("inf"),node) for node in M.intersections.keys() ]
        heapq.heapreplace(heap,(dist_s_g,start))
        ...
        while len(openSet)>0:
            ...
            if current == goal:
                return reconstruct_path(start, cameFrom, current)
            ...
            for neighbor in M.roads[current]:
                ...
                dist_a_b = distance(M.intersections[current], M.intersections[neighbor])
                tentative_gScore = gScore[current] + dist_a_b
...
print(shortest_path(M_40,5,34))
print(shortest_path(M_40,5,5))
print(shortest_path(M_40,8,24))